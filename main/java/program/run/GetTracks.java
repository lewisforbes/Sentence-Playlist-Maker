...
public class GetTracks {
    ...
    public static HashMap<Integer, Track> getTracks(SpotifyApi api, String sentence) {
        ...
        for (int i=0; i<splitSentence.length; i++) {
            attempts = 0;
            output.put(i, getTrack(api, splitSentence[i]));
        }
        return output;
    }
    ...
    private static Track getTrack(SpotifyApi api, String word) {
        ...
        try {
            ...
            if (attempts < maxAttempts) {
                attempts++;
                return getTrack(api, nextAttempt(word));
            }
    ...
    private static boolean onlyLettersEquals(String s1, String s2) {
        return keepOnlyLetters(s1).equalsIgnoreCase(keepOnlyLetters(s2));
    }

    private static String keepOnlyLetters(String input) {
        String output = "";
